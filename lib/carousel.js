...
const getCarouselParams = () => {
    ...
    imageFileNames.map(fileName => {
        ...
        console.log('possible authors', matchingAuthors)
        ...
        if(matchingAuthors.length > 1){
            matchingAuthors.map(authorFileName => {
                ...
                matterResult.data.book_releases.map(book_release => {
                    const releaseID = `${matterResult.data.last_name.toLowerCase()}_${book_release.book_title.toLowerCase().split(' ').join('_')}`;
                    console.log(bookID, releaseID)
                    if(bookID === releaseID){
            ...
            console.log('matching', matchingAuthors)
            const authorID = matchingAuthors[0].replace(/\.md$/, "");
            console.log('authorID', authorID, bookID)
            return carousel.push({authorID, bookID})
        }
        return carousel;
    })
