...
const getSortedImages = () => {
  ...
  const bookID = carouselData.images.map((image) => {
    const imagePath = image.image;
    return imagePath
      .replace("/images/carousel_images/", "")
      .replace(/\.jpg$/, "");
  });
  return bookID;
};
...
const getCarouselParams = () => {
  ...
  carouselData.images.map((image) => {
    ...
    const matchingAuthors = authorFileNames.filter((authorFileName) =>
      authorFileName.includes(authorName)
    );
    ...
    if (matchingAuthors.length > 1) {
      matchingAuthors.map((authorFileName) => {
        ...
        authorData.book_releases.map((book) => {
          const releaseID = `${authorData.last_name.toLowerCase()}_${bookFilenameHander(
            book.book_title
          )}`;
          if (bookID === releaseID) {
...
export { getSortedImages, getCarouselParams };
