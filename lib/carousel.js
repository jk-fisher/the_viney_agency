        ...
        if(matchingAuthors.length > 1){
            matchingAuthors.map(authorFileName => {
                ...
                matterResult.data.book_releases.map(book_release => {
                    const clean_title = book_release.book_title.toLowerCase().split('-').map(element => {
                        return element.trim()
                    });
                    const releaseID = `${matterResult.data.author_last_name.toLowerCase()}_${clean_title.join(' ').split(' ').join('-')}`
                    if(bookID === releaseID){
