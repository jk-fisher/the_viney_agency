...
const getAllBookData = (authorData) => {
    ...
    const bookIDs = authorData.book_releases.map((book) => {
        const bookID = `${authorData.last_name.toLowerCase()}_${book.book_title.toLowerCase().split(' ').join('-')}`;
        return bookID
    })
...
const getAllBookIDs = () => {
    ...
    fileNames.map(fileName => {
        ...
        matterResult.data.book_releases.map(book_release => {
            const bookID = `${lastName}_${book_release.book_title.toLowerCase().split(' ').join('-')}`
